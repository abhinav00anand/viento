...
| Authentication | Bootstrap key + ephemeral session API key |
| Session isolation | Unique logical session ID per runtime registration |
| Replay protection | Directional monotonic sequence numbers + duplicate/gap detection |
| Secrets | API keys never persisted; masked from logs |
| Local server | Ollama remains bound to localhost; never directly exposed |

### Secret Lifecycle

```
ZEPHYR_BOOTSTRAP_KEY (environment)
             │
             ▼
      HELLO.auth_key
             │
      ...
      WELCOME.session_id
             │
             ▼
      SESSION_READY.api_key (1h TTL)
             │
             ▼
   RuntimeState (key stripped)
```
...
Configuration files are stored under `~/.zephyr/` (or custom path via `ConfigManager`). Secrets are always sourced from environment variables.

| Setting | Default | Description |
|---|---:|---|
| `server_url` | `wss://viento.onrender.com/ws/runtime` | Cloud WebSocket URL |
| `cloud_api_url` | `https://viento.onrender.com` | OpenAI-compatible API base |
| `ollama_url` | `http://localhost:11434` | Ollama server URL |
| `max_concurrency` | `2` | Maximum simultaneous inference jobs |
| `max_queue_depth` | `50` | Queue depth before jobs are rejected |
| `heartbeat_interval` | `15s` | Runtime heartbeat cadence |
| `heartbeat_deadline` | `45s` | Server-side offline threshold |
| `job_timeout` | `120s` | Maximum inference duration |
| `token_ttl` | `3600s` | Temporary API key lifetime |
| `reconnect_base_delay` | `1s` | Minimum reconnect delay |
| `reconnect_max_delay` | `30s` | Maximum reconnect delay |
| `reconnect_jitter_factor` | `0.2` | Jitter fraction |
...
- [ ] Ollama / llama.cpp / vLLM running locally
- [ ] Models downloaded
- [ ] `ZEPHYR_BOOTSTRAP_KEY` set in environment
- [ ] `viento doctor` passes
- [ ] Outbound TCP 443 allowed
- [ ] No inbound firewall port required

### Cloud

- [ ] Gateway `wss://` endpoint reachable
- [ ] TLS certificate valid
- [ ] Session registry operational
- [ ] API key TTL configured
- [ ] Rate limiting enabled
- [ ] Gateway health checks active

### Security

- [ ] Bootstrap keys rotated regularly
- [ ] Runtime logs checked for secret leakage
- [ ] API key never written to disk
- [ ] WSS only — never fall back to plain WS
- [ ] Sequence errors trigger session termination
