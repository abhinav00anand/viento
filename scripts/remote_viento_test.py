...
tflops = flops / 1e12
print(
    f"[✓] T4 Matrix Multiplication: {dt / iters * 1000:.2f} ms/iter | Performance: {tflops:.2f} TFLOPS"
)
...
messages = [
    {
        "role": "system",
        "content": "You are Viento AI, an intelligent agent running on the distributed inference mesh.",
    },
...
with torch.no_grad():
    generated_ids = model.generate(
        **model_inputs, max_new_tokens=150, do_sample=True, temperature=0.7
    )
torch.cuda.synchronize()
t_gen_end = time.time()
