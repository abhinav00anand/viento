...
draw.rounded_rectangle([(30, 275), (WIDTH - 30, HEIGHT - 30)], radius=8, fill=(10, 13, 18), outline=BORDER_COLOR)

terminal_lines = [
    ("abhinav@lightning-studio-t4:~$ nvidia-smi", ACCENT_CYAN),
    ("+-----------------------------------------------------------------------------------------+", TEXT_MUTED),
    ("| NVIDIA-SMI 580.173.02             Driver Version: 580.173.02     CUDA Version: 13.0     |", TEXT_WHITE),
    ("| GPU  Name                 Persistence-M | Bus-Id          Disp.A | Volatile Uncorr. ECC |", TEXT_MUTED),
    ("|   0  Tesla T4                       Off |   00000000:00:1E.0 Off |                    0 |", TEXT_WHITE),
...
for idx, (title, lines) in enumerate(steps):
    for f in range(2):  # 2 frames per step for smooth pacing
        ...
        fdraw.text((45, GIF_H - 40), f"Phase {idx + 1} of 5 completed", fill=TEXT_MUTED, font=font_small)
